...
PROMPT_V1_NAME = "donhatminh-rag-prompt-v1"
PROMPT_V2_NAME = "donhatminh-rag-prompt-v2"


# ── 2. Định nghĩa 2 Prompt Templates ──────────────────────────────────────
SYSTEM_V1 = (
    "Bạn là trợ lý AI thân thiện và súc tích. Dựa vào context được cung cấp dưới đây, "
    "hãy trả lời câu hỏi một cách ngắn gọn, trực diện trong khoảng 2-4 câu. "
    "Nếu context không chứa thông tin để trả lời, hãy nói rằng 'Tôi không tìm thấy thông tin này trong tài liệu'.\n\n"
    "Context:\n{context}"
)
...
SYSTEM_V2 = (
    "Bạn là một chuyên gia nghiên cứu và phân tích AI hàng đầu. Dựa vào context được cung cấp dưới đây, "
    "hãy trả lời câu hỏi một cách chi tiết, có cấu trúc chặt chẽ và chuyên nghiệp (3-5 câu). "
    "Hãy trình bày rõ các luận điểm chính và căn cứ theo tài liệu. "
    "Tuyệt đối không suy đoán ngoài context. Nếu không có thông tin, hãy nêu rõ ràng rằng context không đề cập.\n\n"
    "Context:\n{context}"
)
...
def push_prompts_to_hub(client: Client):
    ...
    """
    try:
        url = client.push_prompt(PROMPT_V1_NAME, object=PROMPT_V1, description="V1: Phong cách ngắn gọn, súc tích (2-4 câu)")
        print(f"✅ Đã push V1 → {url}")
    except Exception as e:
        print(f"⚠️  V1 lỗi: {e}")

    try:
        url = client.push_prompt(PROMPT_V2_NAME, object=PROMPT_V2, description="V2: Phong cách chuyên sâu, có cấu trúc (3-5 câu)")
...
def main():
    ...
    log_print("=" * 60)
    log_print("  Bước 2: Prompt Hub & A/B Routing")
    log_print("=" * 60)
    ...
    for i, question in enumerate(SAMPLE_QUESTIONS):
        ...
        if version_tag == "v1":
            ...
            v2_count += 1
        log_print(f"[{i+1:02d}] [prompt-{version_tag}] {question[:55]}...")
        log_print(f"     A: {str(result['answer'])[:75]}...\n")

    log_print(f"\n📊 Routing: V1={v1_count} câu | V2={v2_count} câu | Tổng={len(SAMPLE_QUESTIONS)}")
    log_print("✅ Bước 2 hoàn thành! Kiểm tra Prompt Hub và traces trên LangSmith.")

    log_file = Path(__file__).parent.parent / "evidence" / "02_ab_routing_log.txt"
